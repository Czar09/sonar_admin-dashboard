...
import { getOrders, getSellerOrders, getUsers, getWholeSaleOrders, supabaseAdmin } from '@/utils/supabase-admin'
import { useUser } from '@/utils/useUser';

const SellerOrderComp = () => {
    ...
    const runFun = async () => {
        const order = async () => await getSellerOrders().then((data) => {
                        ...
                        usernum?.map(prod => (
                            <div key={prod.id} className=" p-2">
                                <div className={`p-3 rounded-lg ${prod?.status == 'pending' ? ` bg-yellow-500` : prod.order_status == 'approved' || prod?.status == 'delivered' ? `bg-green-600` : prod?.status == 'cancelled' ? `bg-red-600` : `bg-blue-200`}`} >
                                    <h2 className="text-lg text-gray-900 font-medium title-font mb-4">{prod?.buyer?.["full_name"]} - <small>₹{prod.amount}</small></h2>
                                    <p className="leading-relaxed text-base">Phone Number: {prod?.buyer?.["phone_number"]} </p>
                                    ...
                                    <div className='flex-col'>
                                        <small className='ml-2 text-gray-600 uppercase text-lg space-x-3'>Bought From: <strong>{prod?.seller_products_price?.seller?.full_name}</strong></small>
                                    </div>
                                    <div>
...
export default SellerOrderComp
